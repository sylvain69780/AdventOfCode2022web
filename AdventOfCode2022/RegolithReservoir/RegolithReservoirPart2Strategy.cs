...
{
    public class RegolithReservoirPart2Strategy : IPuzzleStrategy<RegolithReservoirModel>
    {
            ...
            while (true)
            {
                ...
                while (isFreeToMove)
                {
                    ...
                    foreach (var (dx, dy) in RegolithReservoirModel.Directions)
                    {
                        ...
                        if (y < floorPosition && !model.OccupiedPositions!.Contains((x, y)))
                        {
                            newSandPosition = (x, y);
                            break;
                        }
